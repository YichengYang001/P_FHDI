...
void CheckInputdata(double **x_raw, int **r_raw, const int nrow_x, const int ncol_x, ofstream& TestOut) {
	...
	//=====================================================================
    // Check if the input data has potential out of memory issues
    //=====================================================================

	double memory = 0;
	
	memory = (ncol_x*ncol_x * 8) / pow(10,9);

	if (memory > 8) {
		TestOut << "Caution! The memory of correlation matrix is about to exceed the limitation (i.e., 8GB)"<< endl;
	}
