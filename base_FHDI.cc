...
{
	if(nrow<=0 || ncol<=0) 
	{TestOut<<"Error! nrow or ncol<=0! in printing d_debug[][]"<<endl;}
...
void RPrint_Yicheng(double** d_debug, const int nrow, const int ncol, std::ofstream &TestOut_Slave1)
{
	if (nrow <= 0 || ncol <= 0)
	{
		TestOut_Slave1 << "Error! nrow or ncol<=0! in printing d_debug[][]" << endl;
	}
...
//================================================
{
	...
	if(d_sum_w == 0.0) {cout<<"Error! zero sum of weights in wpct"<<endl; return; }
