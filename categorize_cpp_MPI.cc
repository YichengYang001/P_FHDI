...

{
...

	{
		...
		{RPrint("Error! in categorize_cpp, k_one_column is <=1.0!"); return;} //error check
		...
		{ RPrint("Error! n_observed > nrow in categorize()"); return; }
...

{
		...
		if(fabs_FHDI(k_one_column)<=1.0) {RPrint("Error! in categorize_cpp, k_one_column is <=1.0!"); return;} //error check
