...
void Rfn_test_MPI(double** x_raw, int** r_raw, const int nrow_x, const int ncol_x, double* k, 
				   ...
				   const int i_option_imputation, const int i_option_variance, const int i_option_collapsing, const int i_option_SIS_type,
				   int* id, double** z, 
...
//============================================================================
{
	...
	if ((i_option_perform != 4) && (i_collapsing == 0))
	{
	...
	if ((i_option_perform != 4) && (i_collapsing != 0)) {
		...
		bool b_Cell_Make = Cell_Make_Extension_Bigp_cpp(x_raw, r_raw, nrow, ncol, k,
			...
			z, codes,
			rbind_uox_CellMake, rbind_mox_CellMake,
			i_merge, i_collapsing, i_SIS_type, TestOut, d_begin_MPI);
	...
	if ((i_imputation == 1) && (i_collapsing == 0)) //FEFI
		{
	...
	if ((i_imputation == 2) && (i_collapsing == 0)) //FHDI
		{
			//double FHDI_begin = MPI_Wtime();
			s_M = "FHDI";
			FHDI_Extension_cpp(x_raw, z, r_raw,
	...
	if (mynode == 0) {
		...
		if (i_imputation == 2) //FHDI
		{
			...
			FHDI_Extension_cpp(x_raw, z, r_raw,
				...
				List_ord, List_ocsg);
			//testout
			TestOut << " ========= Main FHDI task has successfully finished!" << endl;
		}
	}*/
	...
	*/
	...
	if ((i_collapsing == 0) && (i_variance == 1) && (i_imputation == 1)) {
	...
	if ((i_collapsing == 0) && (i_variance == 1) && (i_imputation == 2)) {
		double** d_cx = New_dMatrix(nrow, ncol);
		Copy_dMatrix(z, nrow, ncol, d_cx);
